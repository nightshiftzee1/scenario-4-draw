...
public class MovingCube : MonoBehaviour {
    ...
    private void SplitCubeOnX(float hang) {
        ...
        float newXSize = lastCube.transform.localScale.x - Mathf.Abs(hang);
        float fallingBlockSize = transform.localScale.x - newXSize;

        float newXPosition = lastCube.transform.position.x + (hang / 2);
    ...
    private void SplitCubeOnZ(float hang) {
        float direction = hang > 0 ? 1f : -1f;
        float newZSize = lastCube.transform.localScale.z - Mathf.Abs(hang);
        float fallingBlockSize = transform.localScale.z - newZSize;

        float newZPosition = lastCube.transform.position.z + (hang / 2);
